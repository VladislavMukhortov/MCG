            ...
            <div class="toggle-wrap nk-block-tools-toggle">
                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        ...
                        <li class="nk-block-tools-opt"><a href="#" class="btn btn-primary" data-toggle="modal" data-target="#modalTop"><em class="icon ni ni-plus-sm"></em><span>Add Managers</span></a></li>
    ...
    <div class="modal-dialog modal-dialog-top" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Add Managers</h5>
            ...
            <div class="modal-body">
                <form action="/managers" method="post">
                    @csrf
                    <input type="hidden" class="form-control" name="user_role" value="Managers">
            ...
            <table class="datatable-init table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{$manager->email}}</td>
                        <td>
                            @if($manager->user_status == 1)
                                Active
                            @elseif($manager->user_status == 0)
                                Inactive
                            @endif
                        </td>
                        <td>{{$manager->role_name}}</td>
                        <td><a href="{{ route('managers.show', $manager->id) }}"><em class="icon ni ni-eye-alt text-primary fs-17px"></em> / <em class="icon ni ni-edit-alt-fill text-primary fs-17px"></em></a></td>
                        <td>
                            <a href="{{ route('managers.destroy', $manager->id) }}" onclick="event.preventDefault(); document.getElementById('delete_{{$manager->id}}').submit();"><em class="icon ni ni-trash-fill text-danger fs-17px"></em></a>
                            <form action="{{ route('managers.destroy', $manager->id) }}" method="post" class="d-none" id="delete_{{$manager->id}}">
                                @method('delete')
                                @csrf
                            </form>
                        </td>
